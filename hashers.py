...
def identify_hasher(encoded):
    ...
    elif all((x in '01') for x in encoded):
...
class MyXORHasher(BasePasswordHasher):
    ...
    def encode(self, password, salt=None):
        ...
        conv_key = self.__conv(self.PASSWORD_KEY)
        n = max(len(conv_password), len(conv_key))
        conv_password += "0" * abs(n - len(conv_password))
        conv_key += "0" * abs(n - len(conv_key))
        return "".join([str(int(conv_password[i]) ^ int(conv_key[i])) for i in range(n)])
    ...
    @staticmethod
    def __conv(str_to_conv):
        return ''.join(format(x,'08b') for x in bytearray(str_to_conv, 'utf-8'))
